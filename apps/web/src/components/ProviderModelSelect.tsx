...
export function ProviderModelSelect({ label, selection, options, loading, onChange }: { label: string; selection: ProviderSelection; options: ProviderModelOption[]; loading: boolean; onChange: (selection: ProviderSelection) => void }) {
  ...
  const visibleOptions = !selectedOption
    ? [...baseOptions, ...(selection.model && !baseOptions.some((option) => `${option.provider}:${option.model}` === value) ? [{ provider: selection.provider, model: selection.model, label: `${selection.model} (unavailable)`, available: false, cloud: selection.model.toLowerCase().split(":").includes("cloud") }] : [])]
    : baseOptions;
