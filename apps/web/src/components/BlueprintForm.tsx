...
export function BlueprintForm({ onSubmit, onCancel }: BlueprintFormProps) {
  ...
  return <form className="space-y-8" onSubmit={submit}>
    ...
    <div className="flex flex-col-reverse justify-end gap-3 sm:flex-row"><button className="button-secondary" type="button" onClick={onCancel}>Cancel</button><button className="button-primary" disabled={saving} type="submit">{saving ? "Validating…" : "Validate & save blueprint"}</button></div>
  </form>;
}
