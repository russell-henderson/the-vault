...
export function PromptPreview({ prompt }: { prompt?: PromptArtifact }) {
  ...
  return <section className="panel overflow-hidden"><div className="flex items-center justify-between gap-4 border-b border-white/10 px-5 py-4"><div><p className="eyebrow">Prompt artifact</p><h2 className="section-title">Codex-ready context</h2></div><div className="flex items-center gap-3"><span className="tag">v{prompt.version}</span><CopyMarkdownButton markdown={prompt.generatedPrompt} /></div></div><pre className="max-h-[520px] overflow-auto whitespace-pre-wrap p-5 font-mono text-xs leading-6 text-slate-300">{prompt.generatedPrompt}</pre></section>;
}
