...
export function Ephemeral({ onOpenSavedMode }: { onOpenSavedMode: () => void }) {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-6 py-12 text-slate-100">
    ...
    <section className="panel mt-6 grid gap-6 p-6 lg:grid-cols-[18rem_1fr]"><aside className="space-y-4"><label className="provider-select"><span>Provider</span><select value={providerKind} onChange={(event) => setProviderKind(event.target.value as ProviderKind)}><option value="ollama">Local Ollama</option><option value="openrouter">OpenRouter</option></select></label>
      ...
      {providerKind === "ollama" ? <label className="provider-select"><span>Local Ollama model</span><input list="local-ollama-models" disabled={loadingModels} value={model} onChange={(event) => setModel(event.target.value)} placeholder={loadingModels ? "Loading local models…" : "Select or enter any installed model"} autoComplete="off" />{models.length > 0 && <datalist id="local-ollama-models">{models.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</datalist>}<small className="normal-case tracking-normal text-slate-500">{models.length > 0 ? `${models.length} local model${models.length === 1 ? "" : "s"} available. Select one or enter any model returned by ollama list.` : "Enter any installed model from ollama list while the browser permission is being fixed."}</small></label> : <label className="provider-select"><span>Model</span><select disabled={!adapter || loadingModels || models.length === 0} value={model} onChange={(event) => setModel(event.target.value)}><option value="">{loadingModels ? "Loading models…" : "Select a model"}</option>{models.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></label>}
