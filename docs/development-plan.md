...
- Windows installer verification must rebuild native SQLite dependencies for the target Electron ABI, inspect that ABI in the packaged artifact, and record at least one clean user-machine smoke test. An unsigned installer remains a pre-release until code signing and broader clean-machine validation are complete.
- The current acceptance baseline is Companion `v1.0.4` pre-release: installer, desktop launch, protocol pairing, local Vault creation, persistence, and Local Ollama catalog behavior have passed user-machine smoke testing. Retire `v1.0.0`–`v1.0.3` installer links.
