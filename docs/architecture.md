...
| Providers | Local Ollama, deterministic mock, server-side OpenRouter embedding adapter, and browser-only Ollama/OpenRouter ephemeral adapters |
...
The Windows Companion `1.0.4` packaging baseline rebuilds `better-sqlite3` for Electron 37's ABI before creating the NSIS installer; it has passed a user-machine smoke test covering install, pairing, local database, and saved workflow. It is the supported pre-release artifact. It remains unsigned until a code-signing certificate and clean-machine release validation are available.
...
- OllamaAiProvider: local HTTP discovery, structured proposal, and execution provider. The installed companion opens the paired saved workspace in an Electron window and keeps the companion API/database alive only while that window is running.
