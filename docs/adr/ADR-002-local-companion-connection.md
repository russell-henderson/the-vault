...
The `1.0.4` companion package explicitly rebuilds the SQLite native module for Electron 37 before NSIS packaging. It is a verified functional baseline, not a production-signed distribution: releases remain pre-releases until signing and additional clean-machine validation are complete.
