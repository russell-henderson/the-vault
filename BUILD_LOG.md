...
## 2026-07-21 — Companion startup entry-point repair

- **Diagnosis:** The packaged `1.0.1` companion included the local API but its shared workspace packages still exported TypeScript source. Electron/Node rejects type stripping under `node_modules`, so startup failed before Fastify could bind.
- **Implementation:** Shared and prompt packages now expose their compiled JavaScript and declaration files. The companion creates `%LOCALAPPDATA%\The Vault Architect` before opening `vault.db`, writes a local diagnostic log if startup ever fails, and displays the exact non-secret error. The hosted Ollama control is an editable catalog-backed field: it lists every discovered local model yet always permits any model name returned by `ollama list`.
